...
L'étude de situations extrêmes

Objectif du projet :
Comprendre ce qui peut provoquer les valeurs extrêmes des indices, l’influence des variables météorologiques, faire des analyses statistiques.
...
Ici on compare les indices Angstrom et FFWI.

#### Estimation empirique de "tail correlation" xi :
...
````{r}
FWI <- cbind.data.frame(indices_McArthur$A,FFwi_values$FFwi)

x <- cbind.data.frame(indindices_McArthurices$grasslandMk5,FWI$`FFwi_values$FFwi`)
x <- na.omit(x)
````
